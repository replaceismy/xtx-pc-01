  ...
  <div class='home-category' @mouseleave="categoryId=null">
    <ul class="menu">
      <li :class="{active:categoryId === item.id}" v-for="item in menuList" :key="item.id" @mouseenter="categoryId=item.id">
        ...
        <!-- 骨架 -->
        <template v-else>
          <XtxSkeloten width="60px" height="18px" style="margin-right:5px" bg="rgba(255,255,255,0.2)" />
          <XtxSkeloten width="50px" height="18px" bg="rgba(255,255,255,0.2)" />
        </template>
      </li>
    </ul>
    <!-- 弹层 -->
...
import XtxSkeloten from '@/components/library/xtx-skeloten.vue'
export default {
  components: { XtxSkeloten },
  name: 'HomeCategory',
  setup () {
